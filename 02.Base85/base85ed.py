...
ENCD_MAP = b"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!#$%&()*+-;<=>?@^_`{|}~"
DECD_MAP = {char: idx for idx, char in enumerate(ENCD_MAP)}
...
def encode(b: bytes):
    ...
    for i in range(0, len(b) - 3, 4):
        ...
        res.extend([ENCD_MAP[val // m % 85] for m in powers85])

    if len(b) % 4:
        val = int.from_bytes(b[len(b) - len(b) % 4 :] + b"\x00" * (4 - len(b) % 4))
        res.extend([ENCD_MAP[val // m % 85] for m in powers85][: len(b) % 4 + 1])
...
def decode(b: bytes):
    ...
    for i in range(0, len(b) - 4, 5):
        val = sum(DECD_MAP[b[i + j]] * powers85[j] for j in range(5))
        if val >= 4294967296:
    ...
    if len(b) % 5:
        ...
        val = sum(DECD_MAP[last_chunk[j]] * powers85[j] for j in range(5))
        if val >= 4294967296:
            raise ValueError(
